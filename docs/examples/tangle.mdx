...
Three operations, and none of them knows about the others. Each strand is a tip with a heading. A rule extrudes the tip one step at a time and turns it toward the curl of a noise field. The rule does not keep a strand away from the others, so the strands cross. A tip that reaches the edge of the frame stops. `oscillate` adds a wobble to each strand. `interlace` looks at every crossing. It decides the strand on top and breaks the other one.
