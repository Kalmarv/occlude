...
Chapter 1 ended here, and this is its code unchanged; the landscape is folded at the top of the editor because nothing in it changes on this page. A shape describes what to draw. You can fill it and hide with it, but you cannot ask it for one point of its edge, and you cannot move that point and leave the rest.
...
This is the finished drawing from the top of the page, and chapter 3 starts from it. One thing to be clear about: the move keeps the connections, so the shore is still a closed loop, but nothing guarantees that the loop does not cross itself. A displacement that is large next to the gap between neighbouring points can fold the outline over, and `polygon` will then fill it with a twist. Small displacements make that unlikely, not impossible; look at the result.
