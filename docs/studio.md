...
## 3D scenes: the camera view

A sketch with a `view(...)` gets a small camera/sketch switch at the bottom right of the sheet. The camera side is an overlay in exactly the preview's box, drawing the model into the scene's own viewport rectangle at the preview's pan and zoom, so switching between the two never shifts the page. It uses Blender's controls: drag orbits, shift+drag pans, ctrl+drag or the wheel zooms, numpad 1/3/7 (ctrl for the opposite side) are the front, right and top presets, numpad 2/4/6/8 step the orbit, and Home frames the model. Presets keep the world up, so a later orbit still turns about Z. **Commit** writes the camera into the sketch's `view(...)` call (or into the scene's camera table for explicit `lineArt3` scenes) without near or far, which the camera factories recompute; an object beyond the default far distance needs an explicit value in source. There is no target editing: moving the camera is the target.
...
A profile's driver is either the EBB board (the older iDraw) or a GRBL controller (the iDraw H family and other G-code machines); both connect over Web Serial from the same Connect button, and the jog, pen, origin, plot, pause and stop controls are the same. On a GRBL machine the pen is the pen library's `penUp`/`penDown` Z heights (or M3/M5 values when the profile's Z mode is off), "Bed origin" writes the controller's work origin, Home uses the switches when the controller has them, and the profile's mirror-Y setting maps the sheet onto the machine exactly as the G-code export does. Time estimates on GRBL are coarse until the machine is calibrated. The servo, seating and lift-map controls are EBB-only and hide otherwise.
