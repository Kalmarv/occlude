...
- `t.settle` takes `point` (a partial record of declared columns or a callback of the splitting parent, which sees its position, attributes and demand): child attributes merged over the inherited copy; `demand` cannot be given, unknown names and non-finite values are refused.
- `t.voronoi(sites, { bounds? })` and the pure `voronoi(points, bounds)` return `Material`: corners as vertices, walls as edges, `faces()` as cells. The result answers `cellOf(site)` and `siteOf(face)`, ownership-checked, for the frozen result and its selections only; edited or extracted material throws a message saying to construct again. Sites given as a point selection (`m.points`, or a filtered part) keep their source: the correspondence answers for that material's vertices and unselected rows have no cell; only bare points become a new material. `connect.triangulate(sites).faces()` are the Delaunay triangles.
...
- Voronoi: area of the faces equals the clipping rectangle to 1e-6 on random, collinear, near-coincident and clipped-away inputs; every wall borders one or two faces; cocircular sites collapse to one shared corner: circumcentres within 1e-9 of the diagram's scale are merged in construction (regular 3- to 50-gons give one centre of degree n, a 6 × 5 lattice twenty corners of degree 4, at unit scale and at 1e-3 scale offset by thousands). Exact-key merging alone left regular polygons crossing by an ulp, which the planar check refused; duplicate sites give one cell owned by the lowest row; a site outside the rectangle owns its clipped cell when any remains. Walls are never merged by rounding: an original endpoint keeps its exact coordinates (a recomputed `p + t·d` differed by one ulp and once produced a touching, which the planar check refused).
...
- Measurement records and their coordinate tuples are frozen (types readonly); non-finite density is absent from the settle raster as it is from face measurement (+Infinity was clamped to 1 before).
