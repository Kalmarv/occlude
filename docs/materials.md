...
Intersection identity and ordering use exact analytical constructions, including when radii vary. Tessellation happens after the union is resolved. At export, adjacent representable coordinates keep extremely narrow boundaries distinct; this roundoff adjustment is checked within the boundary tolerance. Final polygons are checked for crossings and winding changes. Very large overlapping networks can take several seconds. A computation or coordinate-representation limit reports an error instead of silently dropping a boundary.
