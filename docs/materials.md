...
These hints use static TypeScript information. They do not run the sketch, retain intermediate geometry, or enable the Material inspector. Clicking a hint opts into runtime inspection; simply displaying or hovering hints does not. A Shape badge does not claim a particular curve representation, and a vector sampler type does not prove it has field metadata.

In Studio, click a geometry icon (or use **Inspect geometry at cursor** in the editor command palette) to enable inspection and select its capture directly. This reruns the sketch with capture instrumentation. Materials, selections, stations, shapes, paths, faces, and typed scalar/vector fields are supported. Declaration, reference, callback-parameter, and inline-expression icons select their corresponding capture sites, including nested drawing arrays returned from `.map()`. Unsupported write positions, destructured bindings and optional chains remain static rather than changing execution semantics. Vector fields sample automatically when opened; scalar fields offer **Sample field**. Closing the inspector disables inspection, removes overlays, closes Debug and clears its checkboxes. Tutorial editors provide highlighting and hover; open the example in Studio for runtime inspection.

Each capture is associated with its source location and revision, so identical names in different scopes remain separate. Repeated executions of an automatic capture site retain their occurrences and preview them together, subject to the capture budgets; graph occurrences keep separate connectivity and source-row references. A polygon captured inside a loop therefore shows all its polygons. Explicit named `t.inspect()` registrations still replace their previous value. Editing, changing run inputs, or disabling inspection clears the previous live details. Unexecuted declarations are shown as uncaptured; statically typed empty stations remain valid empty captures.

Capture retention is limited to 256 entries and one million point/edge rows across entries; individual capture sites are limited to 250,000 retained rows and 10,000 occurrences. Oversized occurrences retain a summary, and the inspector reports how many were retained rather than silently replacing them with the last value. These are row/handle limits, not a byte budget for attributes or growth history. Station connections are derived only when their preview is requested. Selections show their source row numbers and copied attributes. Station columns distinguish `station.`, `point.` and `edge.` domains; orange/green direction ticks show tangents/normals. Native previews retain arcs and cubics (with control handles), and face previews retain holes, area, perimeter and source walls. For drawing values that reach the returned scene, the default **Post-modifier** overlay uses the renderer's actual visible fragments, including enclosing transforms, smoothing, fills and occlusion. **Pre-modifier** shows the intermediate native geometry with its enclosing placement, before modifiers. Polygon/path inspectors list one shape per row. Click a row to isolate that shape and drill into its drawing options, transforms, and source vertex/path-command table; **All polygons** returns to the complete list and overlay. The **Pre-modifier / Post-modifier** toggle works for the whole collection or the selected shape. Materials and stations remain in source coordinates: arbitrary later placement code cannot be inferred from them. Native captured geometry is limited to 20,000 segments, rendered overlays to 100,000 fragments, and geometry transport to 32 MiB.
