...
The first regular loop sits approximately half a nib inside the visible boundary. Round nibs cannot exactly fill every sharp mathematical corner while remaining entirely inside it; thin features retain visible centerline ink. Solid cleanup subtracts the contour paths’ round-nib footprints from the visible area, then partitions the remnants into triangles and covers them with checked strokes or taps. Triangle boundaries must reproduce the original region, including its holes; matching area alone is insufficient. Each cleanup band must fit wholly in one stroke’s round-nib footprint. Geometric coverage tests avoid drawing extra marks where the actual nib already covers a gap in its polygonal approximation. Native geometry uses a total construction tolerance of `min(0.01 mm, nib / 20, spacing / 10)`, with absolute insets to avoid cumulative drift. If arc offsetting becomes unstable, the fill retries subsequent absolute insets with an integer polygon kernel and retains completed contours. Cleanup handles the remaining uncovered material. If dense contour generation cannot continue, checked local bands complete the remaining area, so complex regions may contain more hatch-like cleanup. Dense generation reserves work for that cleanup after 64,000 contour primitives per component; this does not widen spacing or discard completed contours. The total geometry allowance is four million primitives; overflow and memory/work guards remain, and Studio’s 20-second timeout is the runtime gate. The fill never silently increases final spacing to meet a limit.
