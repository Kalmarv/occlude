...
A 2D study after **Bre Pettis (bre), [38 Monolithic Monuments](https://plotterfiles.com/artwork/38-monolithic-monuments-ighaf)**, shared under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). This is a new Occlude interpretation of the composition and mark-making, not the original generator or a reproduction of one of its SVGs.

Three flat faces suggest each stone: a light front, a dark side and a narrow cap. Bent vertical marks make the faces feel weathered, while the sky's little crosses thin toward the horizon. Every stone is opaque, so it cuts the ground lines behind it. Change the seed for different erosion bands; all the construction stays in 2D.

```ts live paper=Square20
import { sketch, rect, path, group, line, fill, mm, label } from 'occlude';

export default sketch({ aspect: [4, 5], margin: 7, seed: 38 }, (t) => {
  // The fill returns connected cubic marks; Occlude clips them to each face.
  const weathered = (dark) => (region, ctx) => {
    const b = region.bbox, marks = [];
    const gap = ctx.len(mm(dark ? 0.55 : 0.85)) * ctx.coarsen;
    for (let x = b.x; x <= b.x + b.w; x += gap) {
      for (let y = b.y; y < b.y + b.h; y += gap * 3) {
        const bend = gap * (0.25 + 0.7 * t.noise(x / 9, y / 5));
        const end = y + gap * (dark ? 2.9 : 2.4);
        marks.push({ type: 'cubic', x1: x, y1: y,
          cx1: x + bend, cy1: y + gap,
          cx2: x - bend, cy2: end - gap, x2: x, y2: end });
        if (dark) marks.push({ type: 'line', x1: x, y1: y,
          x2: x + gap, y2: y + gap });
      }
    }
    return marks;
  };
  const stone = (x, base, width, height, depth) => {
    const top = base - height, rise = depth * 0.55;
    return group({},
      rect(x, top, width, height, { fill: weathered(false) }),
      path().moveTo(x + width, top).lineTo(x + width + depth, top - rise)
        .lineTo(x + width + depth, base - rise).lineTo(x + width, base)
        .close().build({ fill: weathered(true) }),
      path().moveTo(x, top).lineTo(x + depth, top - rise)
        .lineTo(x + width + depth, top - rise).lineTo(x + width, top)
        .close().build({ fill: fill('hatch', { angle: -28, spacing: mm(0.7) }) }),
    );
  };
  const sky = t.grid({ cols: 54, rows: 65 }).filter(c => c.cy < 46).map(c => {
    const fade = Math.max(0, 1 - c.cy / 46);
    if (!t.chance(fade ** 0.55)) return null;
    const a = 0.65 * fade + 0.12;
    return [line(c.cx - a, c.cy - a, c.cx + a, c.cy + a),
      line(c.cx - a, c.cy + a, c.cx + a, c.cy - a)];
  });
  return [
    sky,
    rect(0, 82, 100, 35, { stroke: false,
      fill: fill('hatch', { angle: 0, spacing: mm(0.7) }) }),
    stone(14, 102, 10, 65, 5),
    stone(33, 105, 12, 81, 6),
    stone(60, 105, 20, 8, 7),
    stone(84, 108, 9, 7, 4),
    label('MONOLITHIC STUDY / AFTER BRE PETTIS', 50, 122, 1.6,
      { align: 'center' }),
  ];
});
```
