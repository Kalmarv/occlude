...
`line(x1, y1, x2, y2)` and `circle(x, y, radius)` are shapes: descriptions of what to draw. The engine turns them into pen strokes on the sheet named under the preview. In the editors on this page, the lines a stage is about are tinted; the rest is what you have already seen.
...
Nothing is cut by hand. A later opaque area hides the earlier marks inside it: the lower half of the sun and the ends of the lowest ruled lines are gone because the filled hill came after them in the array. Two things make a shape an opaque area: a fill, or the option `opaque: true`. A shape with neither, a plain circle or line, hides nothing.
...
**The sun drawn last.** Predict: if the sun moves to the end of the return array, after both hills, what happens to the hatch where the sun's outline crosses it? Change `[sky, sun, farHill, nearHill, pond]` to `[sky, farHill, nearHill, pond, sun]`. Observe: the sun's full circle is drawn across the far hill, and every hatch line under it is still there. Explain: a stroke never hides anything. Hiding is done by opaque area, and an unfilled circle has none; drawing it last only adds its ink.
...
The bank goes last: it must come after the pond to hide the pond's bottom edge. From here on, code carried over from an earlier stage sits in a folded block at the top of the editor; click the arrow in its gutter to unfold it.

</details>

## Where to look things up

Every shape and its options are on [Shapes & layout](#/shapes); the fill patterns and what spacing does to tone on [Fills](#/fills); units, seeds and the four overlap rules, including what happens when ink lands on ink, on [Getting started](#/getting-started). Next, chapter 2: the pond is a shape, so its outline cannot be edited point by point. Material changes that.
