...
Twenty colours of one pen are twenty pens for grouping, pen changes and export, and one library entry. The Paper panel edits the paper library the same way: every sheet there is a name, a size entered in millimetres or inches (the unit is remembered), and the stock's colour from the picker, saved to the server as `papers.json`; `@user/papers` offers exactly those sheets, and the panel's choice is the sheet a sketch without a `paper` declaration is drawn on. A library name works directly too — `stroke: 'micron-03'` resolves from the captured library when the sketch declares no pen of that name — so older sketches keep running. Unknown pen names are an error, so a shared sketch fails loudly rather than plotting with the wrong nib. `DEFAULT_PENS` ships a starter set. Pens with a `reinkMm` budget (paint markers, dip pens, brushes) pause the plot at the first stroke boundary past that many drawn millimetres and wait for Resume; 0 turns it off.

Every run snapshots what it resolved — the sheet and its colour, the pen instances, the seed, the assets and fills — and a saved result keeps that snapshot; editing the library changes later runs only, and the plot-time estimate, the export and the machine all read the run's pens, not the library's. A downloaded sketch bundles the library pens and papers it imports as inline `penModel`/`paperModel` definitions, so it runs anywhere with the definitions it was written against; the Relink button in the studio swaps those back for your libraries' entries of the same names. Two library names that would import as the same identifier (`a-b` and `a_b`) are refused. Headless, the same inputs are explicit: `render(def, { paper, library, seed, assets, fills })`, or a compiled `Execution`. Nothing about a run is read from a session or a global.
...
The G-code `profile` describes the machine the file targets: `bed` (mm), `resolution` (flattening ceiling; GRBL controllers stream a few hundred lines a second, so 0.15 to 0.25 mm keeps curves moving), `travelFeed`, `zMode` (pen by Z moves, else M3/M5), `arcSupport` (emit G2/G3; off by default since many pen controllers take polylines only) and `yAxis`. Coordinates are written as paper millimetres with Y growing downward from the top-left corner (`yAxis: 'down'`); a controller whose home is bottom-left with Y growing upward wants `'up'`, which mirrors Y across the bed, and a top-left home that counts Y negative down the sheet (the iDraw H after homing) wants `'negative'`. Both reflections turn the arcs with them. `penUp` and `penDown` are the machine's pen heights, one pair for the whole machine; when given they override every pen's own. The studio keeps these on each machine profile, and the profile row's "Add preset" starts one for a known machine (the iDraw H A1: 594 × 841 mm, Z pen, 0.2 mm). Verify the orientation with an asymmetric test plot before trusting a new profile.
