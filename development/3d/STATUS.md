...
- Compose project: occlude-3d; image: occlude-3d-dev:latest; localhost port: 5273.
...
Only the dev service should be built/launched. Hardware-browser acceptance is separate.

## Gates

- M0: complete. Isolated dev image built and service launched on localhost:5273. Baseline pnpm check passed all gates: Rust 14.2s, TS 20.1s, library types 4.7s, Studio types 4.3s, docs 12.1s, ink 14.0s, build 35.2s, smoke 2.2s. WASM MD5 f74997a8bc946afc3bbaf4c26c313909. These are local Docker gates, not a remote CI run. Full baseline build log is task-local (baseline-build.log).
- M1–M5: not implemented; no GPU correctness or performance claim yet.
