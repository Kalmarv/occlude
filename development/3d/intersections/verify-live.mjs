...
try{
 ...
 await page.addInitScript(source=>{localStorage.setItem('occlude.sketch',source);const Original=Worker;window.Worker=class extends Original{constructor(...a){super(...a);this.addEventListener('message',e=>{if(e.data.type==='render'){window.reply=e.data;window.count=(window.count??0)+1;}});}};},sources[12]);
 await page.goto(base);await page.waitForFunction(()=>document.querySelector('#status-build')?.textContent.includes('6ab53b6-intersections'));console.log(await page.locator('#status-build').textContent());
 for(const [iteration,i] of [12,24].entries()){
  ...
  results.push({example:`three#${i}`,stats:result.stats,diagnostics:result.diagnostics,svgBytes:result.svg.length});if(i===24){await page.screenshot({path:out+'/example-'+i+'.png'});await writeFile(out+'/example-'+i+'.svg',result.svg);await writeFile(out+'/example-'+i+'.ts',sources[i]);}
 }
