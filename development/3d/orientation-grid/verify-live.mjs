...
try{
 ...
 await page.goto(base);await page.waitForFunction(()=>document.querySelector('#status-build')?.textContent.includes('2bda97a-orientation-grid'));console.log(await page.locator('#status-build').textContent());
 for(let i=0;i<sources.length;i++){
  ...
  results.push({example:`three#${i}`,stats:result.stats,diagnostics:result.diagnostics,svgBytes:result.svg.length});if([12,16,21].includes(i))await page.screenshot({path:out+'/example-'+i+'.png'});
 }
