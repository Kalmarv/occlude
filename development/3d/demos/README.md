...
| --- | --- | --- |
| [Visibility laboratory](visibility-laboratory.ts) | Compare the clean/hidden, dashed/wobbled and contour/wire columns. All three use the same classified scene. Open the browser console for the `visibility-laboratory` report: selected counts, before/after dispatch counters, and three strokes with source parts, lengths and break reasons. Edit the feature predicate to choose another filter. Choose **3D**, orbit, then **Commit view** to reproject all columns. | [SVG](../playwright-visibility-demo/visibility.svg), [screenshot](../playwright-visibility-demo/visibility.png), [report](../playwright-visibility-demo/report.json) |
...
| [Paper composition](paper-composition.ts) | Uses the starter Letter paper and `pigma-01-black` pen imports, two color instances and a one-off pen. Commit a camera, download, and reopen the bundled sketch. | [Portable download](../playwright-paper-composition/paper-composition.ts), [SVG](../playwright-paper-composition/composition.svg), [acceptance note](../PAPER-COMPOSITION.md) |
...
The visibility verifier checks one classified scene, positive visible/hidden/contour selections, a strictly smaller contour selection, unchanged dispatch counters across style construction, populated source-stroke inspection, all required source objects/wire, and successful camera commitment with unchanged source geometry. Both screenshots were visually inspected. The first visibility screenshot exposed labels placed in the wrong units; the final sketch converts the column positions into numeric drawable units (the label API takes numbers).
...
The verifiers start isolated browser contexts, never connect to a plotter, and do not publish sketches or results to the server. Saved-result persistence is separately covered by the main Studio regression evidence. These demos close the named demonstration requirements; remaining reference, precision, performance and final audit work is tracked in [REQUIREMENTS-AUDIT.md](../REQUIREMENTS-AUDIT.md).

Validation for this slice: isolated container `pnpm check` passed Rust/TS tests, library and Studio types, live docs, unchanged docs ink, build, WASM (`615f9d1b7a63396d451fd33aac18b559`) and smoke. Final demo sources were executed by the direct Playwright verifiers against the existing served application. Application code is unchanged. Production container IDs and start times remained unchanged.
