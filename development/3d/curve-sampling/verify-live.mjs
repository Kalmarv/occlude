/** Check the changed intersection demo and existing instance demo through the ordinary Studio worker. */
...
try{
 ...
 await page.addInitScript(source=>{localStorage.setItem('occlude.sketch',source);const timer=window.setTimeout;window.renderTimers=[];window.setTimeout=function(fn,ms,...args){if(ms===60000||ms===20000)window.renderTimers.push(ms);return timer(fn,ms,...args);};const Original=Worker;window.Worker=class extends Original{constructor(...a){super(...a);this.addEventListener('message',e=>{if(e.data.type==='render'){window.reply=e.data;window.count=(window.count??0)+1;}});}};},sources[24]);
 await page.goto(base);await page.waitForFunction(()=>document.querySelector('#status-build')?.textContent.includes('3424244-curve-sampling'));console.log(await page.locator('#status-build').textContent());
 for(const [iteration,i] of [24,25].entries()){
  ...
  results.push({example:`three#${i}`,stats:result.stats,diagnostics:result.diagnostics,svgBytes:result.svg.length});if(i===25){await page.screenshot({path:out+'/example-'+i+'.png'});await writeFile(out+'/example-'+i+'.svg',result.svg);await writeFile(out+'/example-'+i+'.ts',sources[i]);}
 }
 const prior=await page.evaluate(()=>window.count),started=Date.now();
 const delayed=sources[24].replace('async t => {','async t => {\n await new Promise(resolve => setTimeout(resolve, 22000));');
 await page.evaluate(s=>window.__occlude.editor.setValue(s),delayed);
 await page.waitForFunction(n=>window.count>n&&document.querySelector('#status-msg').textContent==='ok',prior,{timeout:60000});
 const elapsedMs=Date.now()-started,timers=await page.evaluate(()=>window.renderTimers);
 assert(elapsedMs>=22000);assert(timers.includes(60000));assert(!timers.includes(20000));
 await writeFile(out+'/timeout.json',JSON.stringify({passed:true,elapsedMs,configuredTimeoutMs:60000,timers},null,2)+'\n');
