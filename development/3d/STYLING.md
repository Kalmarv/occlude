...
The fifth live example in docs/three.md reuses one classification for wobbled visible edges, dashed hidden edges and a separately placed silhouette. Its new ink hash is added deliberately; existing hashes are unchanged. Dash phase still starts per constructed run: continuous source phase across visibility cuts remains unfinished.
