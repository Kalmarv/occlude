...
try{
 ...
 await page.goto(base);await page.waitForFunction(()=>document.querySelector('#status-build')?.textContent.includes('1baa409-corner-domain'));console.log(await page.locator('#status-build').textContent());
 for(let i=0;i<sources.length;i++){
  ...
  results.push({example:`three#${i}`,stats:result.stats,diagnostics:result.diagnostics,svgBytes:result.svg.length});if(i===22){await page.screenshot({path:out+'/example-'+i+'.png'});await writeFile(out+'/example-'+i+'.svg',result.svg);await writeFile(out+'/example-'+i+'.ts',sources[i]);}
 }
