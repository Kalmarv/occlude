...
export default sketchAsync({ ...({ seed: 43, paper: paper({ width: mm(200), height: mm(200) }), margin: 5, pens: {
  ...
  accent: pen({ width: mm(0.25), color: '#A84932' }),
    ...
    "procedural-relief": {"kind":"perspective","eye":[5,7,6],"target":[0,0,0.4],"near":0.1,"far":30,"up":[0,0,1],"fovDegrees":29.9961980673016}
  ...
  surface.points.forEach((p, i) => { const hit = hits.nearest[i]; if (hit && p.position[2] > hit.point[2]) { p.attributes.beforeCeiling = p.position[2]; p.attributes.ceilingAdjusted = true; p.position = hit.point; } });
  ...
  const scene = lineArt3({ id: 'procedural-relief',
