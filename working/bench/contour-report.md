...
Follow-up correctness checks: the real-WASM suite samples 2,005 points across an attached 40 × 0.2 mm finger, including both edges and its tip, against the final decoded plan at nib radius plus 0.01 mm tolerance. Samples are not filtered by an eroded region. Generated ink is validated before finishing modifiers; no contour-specific post-modifier reclip is applied. Sparse fallback hatching retains the requested row spacing, centered on the component, and may replace curved texture with straight rows. Forced complexity fallback and insufficient-budget cases are tested. The measurements above describe the original benchmark build; they have not been rerun as new performance claims for this follow-up.
