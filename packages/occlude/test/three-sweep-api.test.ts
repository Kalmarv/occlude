...
describe('transported profile sweeps',()=>{
 it('produces a shared-rim prism with analytic volume, normals and ray hits',()=>{
  const n=12,r=2,h=3,profile=circle(r,{segments:n}),path=curve([[0,0,0],[0,0,h]]),solid=sweep(profile,path,{caps:true,normal:[1,0,0]});
 ...
 it('rejects undefined frames, singular fields and oversized topology before field evaluation',()=>{
  const profile=circle(),path=curve([[0,0,0],[0,0,1]]);
  ...
  expect(sweep(profile,curve([[0,0,0],[0,0,1],[0,0,2]])).surface.faces.length).toBe(sweep(profile,curve([[0,0,0],[0,0,1],[0,0,0]])).surface.faces.length);
 ...
 it('retains the swept geometry and sampled scale fields across camera commits',async()=>{
  ...
  const definition=sketch({seed:42,pens:{ink:pen({width:mm(.3)})}},()=>{models++;return view(sweep(circle(.3,{segments:8}),curve([[0,0,0],[0,0,1],[1,0,2]]),{caps:true,scale:()=>{scales++;return 1;}}),{camera:orthographic({eye:[5,7,6],span:4})});});
