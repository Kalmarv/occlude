...
describe('connect.unimpeded', () => {
  it('more room is never more edges, and never an edge Delaunay did not have', () => {
    ...
    for (const room of [1, 1.3, 1.7, 2, 2.6, 3.5, 6]) {
      const g = connect.unimpeded(pts, { room });
    ...
    expect(connect.unimpeded(pts, { room: 6 }).edgeCount).toBeLessThan(connect.unimpeded(pts, { room: 1 }).edgeCount / 2);
  });
  ...
  it('up to room 2 it holds the spanning tree, and past 2 it stops doing so', () => {
    ...
    for (const room of [1, 1.5, 2]) {
      const g = edgeSet(connect.unimpeded(pts, { room }));
      for (const e of tree) expect(g.has(e)).toBe(true);
      expect(components(connect.unimpeded(pts, { room }))).toBe(1);
    }
    // Past 2 the lune is big enough to veto edges the tree needed, so the
    // lattice comes apart — which is a real property, not a bug.
    expect(connect.unimpeded(pts, { room: 5 }).edgeCount).toBeLessThan(tree.size);
    expect(components(connect.unimpeded(pts, { room: 5 }))).toBeGreaterThan(1);
  });
  ...
  it('room 1 and room 2 are the two classical answers, checkable by hand', () => {
    ...
    expect(edgeSet(connect.unimpeded(pts, { room: 1 })).has('0-1')).toBe(true);
    expect(edgeSet(connect.unimpeded(pts, { room: 2 })).has('0-1')).toBe(false);
  ...
  it('is deterministic, and refuses what it cannot use', () => {
    ...
    expect(Array.from(connect.unimpeded(pts, { room: 1.6 }).edgeList))
    ...
    const graded = connect.unimpeded(pts, { room: (x) => 1 + (x / 200) * 2.5 });
    ...
    expect(connect.unimpeded(material([[0, 0], [9, 0]]), {}).edgeCount).toBe(1);
    ...
    expect(connect.unimpeded(line, { room: 1 }).edgeCount).toBe(5);
  });
});
