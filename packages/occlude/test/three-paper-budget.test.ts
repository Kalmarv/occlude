...
it('passes the narrowest resolved nib budget per execution and retains it on camera commit',async()=>{
  ...
  for(const width of [.3,.002]){
    const result=await compileSketchAsync(sketch({pens:{ink:pen({width:mm(.3)}), later:pen({width:mm(width)})}},()=>[scene,scene]));
    seen.push(paperBudget3([...result.pens.values()].map(p=>p.width)));
    const committed=await commitCamera3(result,scene,{...camera,span:8});
    seen.push(paperBudget3([...committed.pens.values()].map(p=>p.width)));
  }
