...
describe('isolines',()=>{
  ...
  it('closes latitude loops on a sphere near the analytic circumference',()=>{
    const ball=sphere(1,{segments:32,rings:16}),lines=isolines(ball,c=>c.point.z,[.31,-.62]);
  ...
  it('builds a cross-contour from stored coordinates as one closed loop, and keeps seams honest',()=>{
    const tube=cylinder(1,2,{segments:8}),ring=isolines(tube,c=>c.chart==='side'?(c.uv as number[])[1]:-1,[.5]);
    ...
    const seam=isolines(tube,c=>c.chart==='side'?(c.uv as number[])[0]:-1,[.0625]);
    ...
    const wrap=isolines(tube,c=>c.chart==='side'?(c.uv as number[])[0]:-1,[0]);
  ...
  it('resolves count and spacing level specifications',()=>{
    ...
    expect(isolines(sheet,'h',{count:0}).edges.length).toBe(0);
    expect(isolines(sheet,'h',{spacing:0}).edges.length).toBe(0);
    // One unusable level leaves the others alone.
    expect(new Set(isolines(sheet,'h',[0,Number.NaN,.5]).edges.map(e=>e.level))).toEqual(new Set([0,.5]));
