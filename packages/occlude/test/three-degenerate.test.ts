...
import {box,plane,sphere,cylinder,cone,torus,grid,polyline,curve,circle,mesh,pointCloud,sweep,revolve,isolines,intersections,view,orthographic,instanceOnPoints} from '../src/three/api/index.js';
...
describe('a degenerate input draws nothing, and the sketch keeps rendering',()=>{
 it('makes one empty construction out of every zero size, spacing and count',()=>{
  ...
  for(const made of [circle(0),curve(t=>[t,0,0],{segments:0}),polyline([[1,1,1]]),polyline([])])expect(made.segments.length).toBe(0);
  // Constructions over an empty curve are empty meshes, not failures.
  expect(sweep(circle(0),polyline([[0,0,0],[0,0,1]])).surface.faces.length).toBe(0);
 ...
 it('drops the degenerate element and keeps the rest',()=>{
  ...
  expect(polyline([[0,0,0],[0,0,0],[1,0,0],[2,0,0]]).segments.length).toBe(2);
  ...
  const sheet=plane(4).subdivide(2),left=sheet.faces.filter(f=>f.center[0]<-1),right=sheet.faces.filter(f=>f.center[0]>1);
 ...
 it('answers one sample with a fallback rather than failing the whole field',()=>{
  ...
  expect(new Set(isolines(field,'h',{levels:[0,Number.NaN,.25]}).edges.map(e=>e.level))).toEqual(new Set([0,.25]));
 });
 ...
 it('carries an empty mesh through view, hatch, isolines, sampling, intersections and the plan',async()=>{
  ...
  expect(isolines(nothing,()=>0,{levels:[0]}).edges.length).toBe(0);
  ...
  const blank=await compileSketchAsync(sketch(config,()=>view([nothing],{camera,stroke:'ink',hatch:{spacing:mm(3),stroke:'shade'}})));
  expect(render(blank).raw.frags.length).toBe(0);
  // Beside a real one, the real one is drawn.
  const drawn=await compileSketchAsync(sketch(config,()=>view([nothing,real],{camera,stroke:'ink',hatch:{spacing:mm(3),stroke:'shade'}})));
 ...
 it('keeps the mistakes that are mistakes',()=>{
  ...
  expect(()=>curve(()=>[0,0,0],{segments:100,maxPoints:10})).toThrow('budget');
