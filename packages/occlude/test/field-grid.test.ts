...
import { buildFieldGrids, evaluateGrid, planGrid, type FieldUse } from '../src/fieldGrid.js';
...
describe('field grids: planning', () => {
  it('a paper-aligned grid is windowed to where it is read, on the full grid\'s lattice', () => {
    const full = planGrid([use((x) => x, { shapeFp: { x0: 0, y0: 0, x1: paperW, y1: paperH } })], false, paperW, paperH, unit);
    const win = planGrid([use((x) => x)], false, paperW, paperH, unit);
  ...
  it('a shape-aligned grid is never windowed and its pitch follows the use\'s scale', () => {
    ...
    const p = planGrid([use((x) => x, { aligned: true, m: half, footprint: { x0: 0, y0: 0, x1: 40, y1: 40 } })], false, paperW, paperH, unit);
    expect(p.ci0).toBe(0);
    expect(p.cj0).toBe(0);
...
describe('field grids: evaluation', () => {
  it('samples the callable on the planned lattice and writes the engine record', () => {
    ...
    const plan = planGrid([use(fn)], false, paperW, paperH, unit);
  ...
  it('a vector field fills both components from one evaluation per sample', () => {
    ...
    const plan = planGrid([use(fn, { kind: 'vx' })], true, paperW, paperH, unit);
  ...
  it('uses of one field share one grid; a second field gets its own', () => {
    ...
    const data = buildFieldGrids(uses, idOf, paperW, paperH, unit, inner);
