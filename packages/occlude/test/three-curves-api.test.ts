...
import {polyline,curve,circle,box,view,orthographic,perspective} from 'occlude/3d';
...
describe('owned 3D curve geometry',()=>{
 it('owns raw points, shares closed seams and retains only point/edge capabilities',()=>{
  ...
  const path=polyline(points,{closed:true});points[0][0]=99;
  ...
  expect(polyline([[0,0,0],[1,0,0],[1,1,0]],{closed:true}).surface).toEqual(path.surface);
 });
 ...
 it('uses frozen edit passes, rejects foreign selections and retains history',()=>{
  const original=polyline([[0,0,0],[1,0,0]]).attribute('gain',2);
  let escaped:any;
 ...
 it('uses the existing visibility engine with analytic wire/box intervals and no curve occluders',()=>{
  const path=polyline([[-2,0,-1],[2,0,-1]]).edgeAttribute('ink','wire').withKey('path');
 ...
 it('retains a curve interpretation across camera commits without resampling the model',async()=>{
  ...
  const definition=sketch({seed:42,pens:{ink:pen({width:mm(.3)})}},()=>{models++;return view(curve(t=>{samples++;return [Math.cos(t*6),Math.sin(t*6),t];},{segments:16}),{camera,stroke:'ink'});});
