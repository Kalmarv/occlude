...
import { connect, curve, material, warp, type Material } from '../src/index.js';
...
describe('warp', () => {
  it('a cage that has not moved moves nothing, and an affine cage is that affine map', () => {
    ...
    const still = warp(grid, { from: square(), to: square() });
    ...
    const scaled = warp(grid, { from: square(), to: moved });
    for (let i = 0; i < grid.n; i++) {
  ...
  it('a corner takes its corner with it, and an edge stays on its edge', () => {
    ...
    const onCorner = warp(material([[100, 100]]), { from: square(), to: pulled });
    ...
    const onEdge = warp(material([[50, 0]]), { from: square(), to: pulled });
    expect(at(onEdge, 0)[0]).toBeCloseTo(50, 9);
    expect(at(onEdge, 0)[1]).toBeCloseTo(0, 9);
    const far = warp(material([[100, 50]]), { from: square(), to: pulled });
  ...
  it('keeps the drawing a drawing: structure, columns and shape', () => {
    ...
    const w = warp(ring, { from: square(), to: pulled });
    ...
    const gaps = rulings.map((r) => warp(r, { from: square(), to: pulled })).map((r) => r.y[0]);
  ...
  it('refuses a cage it cannot use', () => {
    ...
    const asMaterial = warp(m, { from: material(square()), to: material(square()) });
    expect(at(asMaterial, 0)).toEqual([10, 10]);
    // Deterministic.
    const twice = () => warp(material([[33, 71]]), { from: square(), to: [[0, 0], [100, 0], [140, 130], [0, 100]] });
