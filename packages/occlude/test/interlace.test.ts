...
describe('interlace', () => {
  ...
  it('`over` decides which one, and it is the only thing that does', () => {
    ...
    const aOver = x.interlace({ gap: 10, over: () => true });
    const bOver = x.interlace({ gap: 10, over: () => false });
    ...
    const seen: number[][] = [];
    x.interlace({ gap: 4, over: (c) => { seen.push([c.x, c.y]); return true; } });
  ...
  it('a curve with nothing crossing it loses nothing, seam included', () => {
    ...
    expect(ink(apart.interlace({ gap: 6 }))).toBeCloseTo(ink(apart), 9);
    // And one that genuinely does cross still loses exactly one gap.
    expect(ink(cross().interlace({ gap: 6 }))).toBeCloseTo(ink(cross()) - 6, 6);
  });

  it('is deterministic, and refuses what it cannot use', () => {
    const x = cross();
    expect(Array.from(x.interlace({ gap: 7 }).x)).toEqual(Array.from(x.interlace({ gap: 7 }).x));
    // A gap below zero cuts nothing away; a missing one is still a mistake.
    expect(Array.from(x.interlace({ gap: -1 }).x)).toEqual(Array.from(x.interlace({ gap: 0 }).x));
    expect(() => x.interlace({} as never)).toThrow(/non-negative length/);
    expect(() => x.interlace({ gap: 4, over: 1 as never })).toThrow(/must be a function/);
    // Nothing to weave is not an error.
    expect(material([]).interlace({ gap: 4 }).edgeCount).toBe(0);
    expect(ink(curve([[0, 0], [9, 9]]).interlace({ gap: 4 }))).toBeCloseTo(Math.hypot(9, 9), 6);
  });
});
