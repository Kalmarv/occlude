/**
 * Hershey Roman Duplex — Roman duplex: two strokes to a stem, so it reads heavier than simplex.
 *
 * The bytes below are rowmand.jhf from the Usenet Hershey font distribution,
 * verbatim except for the TypeScript string escaping of backticks. The
 * whole distribution notice is beside this file in HERSHEY-NOTICE.txt and
 * its acknowledgements are required wherever the data travels:
 *
 ...
 *
 * Public domain, free for any purpose commercial or otherwise, on those
 * two acknowledgements and on not re-creating the NTIS distribution
 * format. Source: https://github.com/kamalmostafa/hershey-fonts
 * (hershey-fonts/rowmand.jhf), whose COPYING keeps the .jhf data under this
 * notice and not under the repository's GPL code licence. No line of the
 * Inkscape Hershey Text extension was read or used.
 */

import type { Font } from '../strokeFont.js';
import { lazyFont } from './lazy.js';

/** rowmand.jhf, as distributed: one glyph per line, ASCII order from space. */
export const ROWMAND_JHF = ` 2699  1JZ
...
/** Roman duplex: two strokes to a stem, so it reads heavier than simplex. */
export const hersheyDuplex: Font = lazyFont('Hershey Roman Duplex', () => ROWMAND_JHF);
