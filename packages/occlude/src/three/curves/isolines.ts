...
export interface IsolineOptions3 {
   ...
   *
   * Every row that survives is the row full construction gave, field for
   * field, its cumulative `range` included: a mixed chain measures its hidden
   * runs (the phase points below) so the surviving records sit where they
   * always did along the chain's parameter. What a certified run does NOT
   * leave behind is geometry, so a consumer that walks the chain's emitted
   * records — the stroke stage's reference polyline, and the dash phase and
   * jitter measured along it — sees the surviving runs and not the whole
   * chain. On a partly hidden ring a dash therefore starts somewhere else.
   * Same ink, different pattern; Caleb's call, 2026-09-21. */
...
export function isolines3(input:Surface3,values:ArrayLike<number>,levels:readonly number[],options:IsolineOptions3={}):IsolineResult3 {
  ...
  for(const [li,list] of byLevel){
    ...
    const emit=(chain:{segment:number;forward:boolean}[])=>{
      ...
      // A chain that enters the certified faces and comes back out measures
      // the deferred runs inside it, so every surviving record keeps the
      // cumulative range full construction gave it. The records themselves
      // are still not emitted — that is the report's §3.3 obstruction, and
      // what it costs is the chain reference downstream, not this parameter.
