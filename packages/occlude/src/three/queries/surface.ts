...
export function triangleBarycentric3(triangle:Triangle3,p:Vec3):Vec3 {
  ...
  const denominator=orient(a,b,c);if(denominator===0)throw new Error('surface query encountered a degenerate triangle');
...
export function rayTriangle3(triangle:Triangle3,query:RayQuery3):{point:Vec3;barycentric:Vec3;distance:number}|null {
  ...
  return barycentric.every(v=>v>=0)?{point,barycentric,distance:t}:null;
}
...
export function nearestTriangle3(triangle:Triangle3,p:Vec3):{point:Vec3;barycentric:Vec3;distance:number} {
  ...
  if(!(nn>0)||!Number.isFinite(nn))throw new Error('surface query triangle has degenerate or unrepresentable area');
  const projected=sub3(p,mul3(n,dot3(n,sub3(p,a))/nn)),barycentric=triangleBarycentric3(triangle,projected);
  if(barycentric.every(v=>v>=0))return {point:projected,barycentric,distance:Math.hypot(...sub3(p,projected))};
