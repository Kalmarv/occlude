...
interface Node { bounds: Bounds3; left?: Node; right?: Node; indices?: readonly number[] }
...
/** Deterministic median BVH, O(n log² n) build, no segment×triangle storage. */
export class ProjectedIndex3 {
  private root: Node | null;
  constructor(private bounds: readonly Bounds3[]) {
    const build = (indices: number[]): Node => {
      const box: [number, number, number, number] = [Infinity, Infinity, -Infinity, -Infinity];
      for (const i of indices) { const b = bounds[i]; box[0] = Math.min(box[0], b[0]); box[1] = Math.min(box[1], b[1]); box[2] = Math.max(box[2], b[2]); box[3] = Math.max(box[3], b[3]); }
      if (indices.length <= 8) return { bounds: box, indices };
      ...
      return { bounds: box, left: build(indices.slice(0, split)), right: build(indices.slice(split)) };
    };
    this.root = bounds.length ? build(bounds.map((_, i) => i)) : null;
  }
  *query(bounds: Bounds3): Generator<number> {
    const stack = this.root ? [this.root] : [];
    while (stack.length) {
      const node = stack.pop()!; if (!intersects(bounds, node.bounds)) continue;
      if (node.indices) { for (const i of node.indices) if (intersects(bounds, this.bounds[i])) yield i; }
