...
export function* candidatePairs3(snapshot: FeatureSnapshot3): Generator<{ feature: number; occluder: number; pair: VisibilityPair3 }> {
  for (let i = 0; i < snapshot.features.length; i++) {
    ...
    for (const j of snapshot.index.query(bounds)) {
      ...
      if (!feature.support.includes(occluder.id)) yield { feature: i, occluder: j, pair: { a: feature.a, b: feature.b, volume: occluder.volume } };
    }
  }
}
...
export async function classifySceneGpu3(snapshot: FeatureSnapshot3, gpu: GpuIntervals3, options: { signal?: AbortSignal; pairCapacity?: number; maxCandidates?: number; parameterTolerance?: number } = {}): Promise<ClassifiedScene3> {
  ...
  records.forEach((runs, feature) => {
    ...
    runs.forEach((r,i) => {
      const interval = refine.has(i) ? hiddenInterval3(r.pair.a,r.pair.b,r.pair.volume) : r.interval;
