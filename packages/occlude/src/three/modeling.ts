...
import { SurfaceQueries3, type RayQuery3, type NearestQuery3, type SurfaceHit3 } from './queries/surface.js';
...
export interface ModelingStats3 { readonly operation: 'deform' | 'query'; readonly backend: 'cpu' | 'gpu'; readonly dispatches: number; readonly transferBytes: number }
...
export function bindModeling3(exec: Execution, scope?: { signal?: AbortSignal; compute3?: SceneCompute3; isOpen?: () => boolean }) {
  ...
  return {
    ...
    querySurface3(surface: Surface3, queries: SurfaceQueryInput3): Promise<SurfaceQueryResult3> {
      check();
      const source = new SurfaceQueries3(surface), captured = structuredClone(queries);
      return (async () => {
        const compute = scope!.compute3;
