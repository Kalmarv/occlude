...
const reserved=new Set(['id','index','x','y','z','attributes','provenance','vertices','normal','center','area','a','b','length','source','sample']);
...
function setPoints<P extends Attributes3>(surface:Surface3,name:string,field:Field<PointRow<P>,Attribute3>):Surface3 {
  attributeName(name);const values=pointRows<P>(surface).map(row=>attributeValue(evaluate(field,row)));
...
function displaced<P extends Attributes3>(surface:Surface3,field:Field<PointRow<P>,Vec3>):Surface3 {
  const points=pointRows<P>(surface).map((row,i)=>{const delta=evaluate(field,row);finite3(delta);return {...surface.points[i],position:add3(surface.points[i].position,delta)};});
...
export class PointGeometry<P extends Attributes3={}> {
  ...
  constructor(surface:Surface3,options:GeometryOptions={}){checkOptions(options);validateAttributes(surface);this.surface=snapshotSurface3(surface);this.key=checkedKey(options.key);Object.freeze(this);}
  ...
  attribute<Name extends string,Value extends Attribute3>(name:Name,field:Field<PointRow<P>,Value>):PointGeometry<Omit<P,Name>&Record<Name,Value>>{return new PointGeometry(setPoints(this.surface,name,field),this);}
  displace(field:Field<PointRow<P>,Vec3>):PointGeometry<P>{return new PointGeometry(displaced(this.surface,field),this);}
  translate(offset:Vec3):PointGeometry<P>{return new PointGeometry(transformSurface3(this.surface,{translate:offset}),this);}
...
export class Mesh<P extends Attributes3={},E extends EdgeAttributes={},F extends Attributes3={}> {
  ...
  faces():Collection<FaceRow<F>,Mesh<P,E,F>>{
    const measures=measureFaces3(this.surface);
    const rows=measures.map((m,index)=>Object.freeze({...m.attributes,id:m.id,index,vertices:this.surface.faces[index].vertices,normal:m.normal,center:m.center,area:m.area,attributes:m.attributes,provenance:this.surface.faces[index].provenance})) as unknown as readonly FaceRow<F>[];
    return new Collection(this.surface,'face',rows,indices=>{
      const used=[...new Set(indices.flatMap(i=>this.surface.faces[i].vertices))].sort((a,b)=>a-b),mapping=new Map(used.map((v,i)=>[v,i]));
      const faceMap=new Map(indices.map((v,i)=>[v,i]));
      const surface=assembleSurface3(used.map(v=>this.surface.points[v]),indices.map(i=>({...this.surface.faces[i],vertices:this.surface.faces[i].vertices.map(v=>mapping.get(v)!)})),this.surface.triangles.filter(t=>faceMap.has(t.face)).map(t=>({face:faceMap.get(t.face)!,vertices:t.vertices.map(v=>mapping.get(v)!) as [number,number,number]})),this.surface);
      return new Mesh(surface,{transfers:this.transfers});
    });
  }
  attribute<Name extends string,Value extends Attribute3>(name:Name,field:Field<PointRow<P>,Value>,options:{transfer?:'interpolate'|'nearest'}={}):Mesh<Omit<P,Name>&Record<Name,Value>,E,F>{
    return new Mesh<Omit<P,Name>&Record<Name,Value>,E,F>(setPoints(this.surface,name,field),{...this,history:[],transfers:{...this.transfers,[name]:options.transfer??'interpolate'}});
  }
  edgeAttribute<Name extends string,Value extends Attribute3>(name:Name,field:Field<EdgeRow<E,P>,Value>):Mesh<P,Omit<E,Name>&Record<Name,Value>,F>{
    attributeName(name);const values=this.edges.map(row=>attributeValue(evaluate(field,row))),surface=cloneSurface3(this.surface);
  ...
  faceAttribute<Name extends string,Value extends Attribute3>(name:Name,field:Field<FaceRow<F>,Value>):Mesh<P,E,Omit<F,Name>&Record<Name,Value>>{
  ...
  faceAttributes<A extends Attributes3>(field:Field<FaceRow<F>,A>):Mesh<P,E,Omit<F,keyof A>&A>{
    const values=this.faces().map(row=>evaluate(field,row)),surface=cloneSurface3(this.surface);
    values.forEach((attrs,i)=>{for(const [name,value] of Object.entries(attrs)){attributeName(name);surface.faces[i].attributes[name]=attributeValue(value);}});
  ...
  displace(field:Field<PointRow<P>,Vec3>):Mesh<P,E,F>{return new Mesh(displaced(this.surface,field),{...this,history:[]});}
  ...
  steps(count:number,rule:MeshRule<P,E,F>,...passesAndOptions:(MeshRule<P,E,F>|StepsOptions)[]):Mesh<P,E,F>{
    ...
    const passes=[rule,...passesAndOptions.filter((p):p is MeshRule<P,E,F>=>typeof p==='function')];
    let current=new Mesh<P,E,F>(this.surface,{...this,history:[]});const history:MeshSnapshot<P,E,F>[]=[];
...
export class MeshEdit<P extends Attributes3,E extends EdgeAttributes,F extends Attributes3> {
  private active=true;private readonly points:SurfacePoint3[];
  constructor(private readonly input:Mesh<P,E,F>){this.points=input.surface.points.map(p=>({...p,position:[...p.position] as Vec3,attributes:structuredClone(p.attributes)}));}
  private check(selection:Collection<PointRow<P>,unknown>):void {if(!this.active)throw new Error('mesh editor is closed');if(selection.domain!=='point'||selection.source!==this.input.surface)throw new Error('point selection belongs to another mesh revision; select from current.points');}
  move(selection:Collection<PointRow<P>,unknown>,field:Field<PointRow<P>,Vec3>):void{
    this.check(selection);const deltas=selection.map(p=>{const delta=evaluate(field,p);finite3(delta);return delta;});
    selection.indices.forEach((i,j)=>this.points[i].position=add3(this.points[i].position,deltas[j]));
  }
  finish(iteration:number):Mesh<P,E,F>{if(!this.active)throw new Error('mesh editor is closed');this.active=false;return new Mesh(assembleSurface3(this.points,this.input.surface.faces,this.input.surface.triangles,this.input.surface),{...this.input,iteration,history:[]});}
  close():void{this.active=false;}
}
