...
export function captureSurfaceMapping(mesh:Mesh<any,any,any,any>,pattern:Material|readonly Material[],options:SurfaceMappingOptions={}) {
  ...
  const captured=frame===undefined?[]:patterns.flatMap(p=>{
    ...
    return [new Material(x, y, columns(p.attrs), p.edgeList.slice(), { iteration: 0, history: [], edgeAttrs: columns(p.edgeAttrs), transfers: {...p.transfers}, edgeTransfers: {...p.edgeTransfers}, ids: {points:Float64Array.from(p.pointIds),edges:Float64Array.from(p.edgeIds)} })];
  });
