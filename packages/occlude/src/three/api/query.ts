...
export interface NearestBatchOptions<P extends Attributes3> {readonly position?:Field<PointRow<P>,PointLike3>;readonly within?:Field<PointRow<P>,number>}
export interface RayBatchOptions<P extends Attributes3> {readonly origin?:Field<PointRow<P>,PointLike3>;readonly direction:Field<PointRow<P>,Vec3>;readonly near?:Field<PointRow<P>,number>;readonly far?:Field<PointRow<P>,number>}
export interface SegmentBatchOptions<P extends Attributes3> {readonly from?:Field<PointRow<P>,PointLike3>;readonly to:Field<PointRow<P>,PointLike3>}
...
function rows<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>):readonly PointRow<P>[]{if(!(selection instanceof Collection)||selection.domain!=='point')throw new Error('query batches require a point collection');return Object.freeze([...selection]);}
...
function nearestInput<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:NearestBatchOptions<P>){const source=rows(selection),queries=source.map(p=>({point:position3(options.position===undefined?p:evaluate(options.position,p)),maxDistance:options.within===undefined?undefined:evaluate(options.within,p)}));queries.forEach(validateNearest3);return {source,queries};}
function rayInput<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:RayBatchOptions<P>){const source=rows(selection),queries=source.map(p=>({origin:position3(options.origin===undefined?p:evaluate(options.origin,p)),direction:position3(evaluate(options.direction,p)),near:options.near===undefined?undefined:evaluate(options.near,p),far:options.far===undefined?undefined:evaluate(options.far,p)}));queries.forEach(validateRay3);return {source,queries};}
function segmentInput<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:SegmentBatchOptions<P>){
...
function segmentResults<P extends Attributes3,F extends Attributes3>(prepared:PreparedQuery<F>,input:ReturnType<typeof segmentInput<P>>,out:SurfaceQueryResult3){
...
export class QueryBatch<F extends Attributes3> {
  ...
  nearest<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:NearestBatchOptions<P>={}):readonly QueryResult<PointRow<P>,SurfaceHit<F>>[]{const input=nearestInput(selection,options);return results(input.source,convertNearest(this.prepared,run(this.prepared,{nearest:input.queries}).nearest));}
  rays<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:RayBatchOptions<P>):readonly QueryResult<PointRow<P>,RayHit<F>>[]{const input=rayInput(selection,options);return results(input.source,convertRays(this.prepared,run(this.prepared,{rays:input.queries}).rays,input.queries.map(q=>q.origin)));}
  segments<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:SegmentBatchOptions<P>):readonly QueryResult<PointRow<P>,RayHit<F>>[]{const input=segmentInput(selection,options);return segmentResults(this.prepared,input,run(this.prepared,{segments:input.segments,nearest:input.nearest}));}
}
...
export class AsyncQueryBatch<F extends Attributes3> {
  ...
  async nearest<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:NearestBatchOptions<P>={}):Promise<readonly QueryResult<PointRow<P>,SurfaceHit<F>>[]>{const input=nearestInput(selection,options),out=await this.host.querySurface3(this.prepared.target.surface,{nearest:input.queries});return results(input.source,convertNearest(this.prepared,out.nearest));}
  async rays<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:RayBatchOptions<P>):Promise<readonly QueryResult<PointRow<P>,RayHit<F>>[]>{const input=rayInput(selection,options),out=await this.host.querySurface3(this.prepared.target.surface,{rays:input.queries});return results(input.source,convertRays(this.prepared,out.rays,input.queries.map(q=>q.origin)));}
  async segments<P extends Attributes3>(selection:Collection<PointRow<P>,unknown>,options:SegmentBatchOptions<P>):Promise<readonly QueryResult<PointRow<P>,RayHit<F>>[]>{const input=segmentInput(selection,options),out=await this.host.querySurface3(this.prepared.target.surface,{segments:input.segments,nearest:input.nearest});return segmentResults(this.prepared,input,out);}
}
