...
export function captureIntersections(a:IntersectionInput,b:IntersectionInput,options:IntersectionOptions={}) {
 const settings=structuredClone(options),maxPairs=settings.maxPairs??4096;
 if(!Number.isSafeInteger(maxPairs)||maxPairs<0)throw new Error('intersection pair budget must be a nonnegative integer');
 ...
 if(na+nb>(budget.graph?.maxSources??100000))throw new Error('intersections exceed source budget');
 ...
 if(size(a,'points')+size(b,'points')>(budget.contacts?.maxInputPoints??500000)||size(a,'triangles')+size(b,'triangles')>(budget.contacts?.maxInputTriangles??250000))throw new Error('intersection input exceeds triangle/point budget');
...
export function* intersectionConstructionJob(captured:ReturnType<typeof captureIntersections>) {
 ...
 for(let ai=0;ai<left.length;ai++)for(let bi=0;bi<right.length;bi++){
  ...
  const result=yield*intersectionsJob3(a.binding,b.binding,{
   contacts:{...contactsBudget,maxCandidates:(contactsBudget.maxCandidates??1000000)-stats.candidates,maxContacts:(contactsBudget.maxContacts??250000)-stats.contacts,maxContactPoints:(contactsBudget.maxContactPoints??1000000)-stats.contactPoints,maxExactBytes:(contactsBudget.maxExactBytes??64000000)-stats.exactBytes},
   arrangement:{...arrangementBudget,maxCandidates:(arrangementBudget.maxCandidates??1000000)-stats.arrangementCandidates,maxSupportCandidates:(arrangementBudget.maxSupportCandidates??1000000)-stats.supportCandidates},
   graph:{...graphBudget,maxNodes:(graphBudget.maxNodes??250000)-nodes.length,maxSegments:(graphBudget.maxSegments??250000)-segments.length},
  });
  stats.pairs++;stats.candidates+=result.stats.candidates;stats.arrangementCandidates+=result.stats.arrangementCandidates;stats.supportCandidates+=result.stats.supportCandidates;
  ...
  for(let i=0;i<network.segments.length;i++){
   ...
   if(supportCount>(graphBudget.maxSupports??1000000))throw new Error('surface curve graph exceeds support budget');
  ...
  for(let i=0;i<network.nodes.length;i++){
   ...
   supportCount+=supports?.length??0;if(supportCount>(graphBudget.maxSupports??1000000))throw new Error('surface curve graph exceeds support budget');
   graphBytes+=row.exact.reduce((n,s)=>n+s.length*2,0)+row.supports.reduce((n,s)=>n+s.weights.reduce((n,v)=>n+v.length*2,0),0);
   if(graphBytes>(graphBudget.maxExactBytes??64000000))throw new Error('surface curve graph exceeds exact-coordinate byte budget');
