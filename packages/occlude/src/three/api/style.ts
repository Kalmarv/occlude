...
/** One place to say how things are drawn: `style(geometry, { pen, fillPen,
 * creaseAngle, hatch })` returns the same shape it was given, a value or a list of
 * them (nested lists flatten as `view` does), with the fields named set and
 * the rest kept. Curves take only `pen`. Factory options are the same
 * style at birth; the view's `pen` and `hatch` are the default for anything
 * unstyled. */
...
function styleOne(value:unknown,s:Style3):Styleable {
  ...
  if(value instanceof SurfaceCurves){
    if(s.fillPen!==undefined||s.creaseAngle!==undefined||s.suggestive!==undefined||s.hatch!==undefined)throw new Error('curves take only a pen: fillPen, creaseAngle, suggestive and hatch belong to meshes');
