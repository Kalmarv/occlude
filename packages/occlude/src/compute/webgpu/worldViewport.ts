...
const shader = /* wgsl */ `
struct View { right:vec4f, up:vec4f, back:vec4f, eye:vec4f, viewport:vec4f }
...
fn project(p:vec3f)->vec4f {
  ...
  if(view.viewport.z>0.5){return vec4f(p.x*view.right.w,p.y*view.up.w,far/(near-far)*p.z+far/(near-far)*near,-p.z);}
  return vec4f(p.x*view.right.w,p.y*view.up.w,(-p.z-near)/(far-near),1.0);
}
 ...
 * orbit updates one 80-byte camera uniform. Final vectors use the geometric
...
export class GpuWorldViewport3 {
  ...
  constructor(private readonly device:GPUDevice,private readonly canvas:HTMLCanvasElement|OffscreenCanvas,format:GPUTextureFormat){
    ...
    this.uniform=device.createBuffer({label:'Construction camera',size:80,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});
  ...
  get stats(){return {geometryUploads:this.uploads,geometryUploadBytes:this.uploadBytes,uniformUploadBytes:this.frames*80,residentBufferBytes:this.bytes};}
  private upload(triangles:readonly Triangle3[],wires:readonly(readonly[Vec3,Vec3])[]):void{
    ...
    this.vertices?.destroy();this.edges?.destroy();this.bytes=80;
  ...
  draw(frame:CameraFrame3,triangles:readonly Triangle3[],wires:readonly(readonly[Vec3,Vec3])[]):void{
    ...
    const y=c.kind==='perspective'?1/Math.tan(c.fovDegrees*Math.PI/360):2*this.scale/c.span;
    const uniform=new Float32Array([...frame.right,y/aspect,...frame.up,y,...frame.back,c.near/this.scale,...c.eye.map((v,k)=>(v-this.center[k])/this.scale),c.far/this.scale,this.canvas.width,this.canvas.height,c.kind==='perspective'?1:0,0]);
