...
export interface QueryBatch3 {readonly hits:readonly(SurfaceHit3|null)[];readonly stats:{queries:number;dispatches:number;refinements:number;transferBytes:number;wallMs:number}}
...
export class GpuSurfaceQueries3 {
  ...
  static async create(device:GPUDevice,source:SurfaceQueries3,options:{memoryBudgetBytes?:number;batchSize?:number}={}):Promise<GpuSurfaceQueries3>{
    ...
    const triangles=size?device.createBuffer({size,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}):null;if(triangles&&!forceCpu)device.queue.writeBuffer(triangles,0,packed);
    return new GpuSurfaceQueries3(device,source,triangles,{rays,nearest},origin,scale,capacity,forceCpu);
  }
  rays(queries:readonly RayQuery3[],options:{signal?:AbortSignal;maxTests?:number}={}):Promise<QueryBatch3>{queries.forEach(validateRay3);return this.submit('rays',structuredClone(queries),options);}
  segments(segments:readonly(readonly[Vec3,Vec3])[],options:{signal?:AbortSignal;maxTests?:number}={}):Promise<QueryBatch3>{return this.rays(segments.map(([a,b])=>({origin:a,direction:[b[0]-a[0],b[1]-a[1],b[2]-a[2]],near:0,far:1})),options);}
  nearest(queries:readonly NearestQuery3[],options:{signal?:AbortSignal;maxTests?:number}={}):Promise<QueryBatch3>{queries.forEach(validateNearest3);return this.submit('nearest',structuredClone(queries),options);}
  private submit(kind:'rays'|'nearest',queries:readonly(RayQuery3|NearestQuery3)[],options:{signal?:AbortSignal;maxTests?:number}){
    ...
    const job=this.tail.then(()=>this.run(kind,queries,options.signal));this.tail=job.catch(()=>{});return job;
  }
  private async run(kind:'rays'|'nearest',queries:readonly(RayQuery3|NearestQuery3)[],signal?:AbortSignal):Promise<QueryBatch3>{
    ...
    if(!this.triangles||this.forceCpu){for(const q of queries){check();hits.push(cpu(q));stats.refinements++;}stats.wallMs=performance.now()-start;return {hits,stats};}
    const capacity=Math.min(this.capacity,queries.length);if(!capacity)return {hits,stats};
    ...
    try {
      for(let offset=0;offset<queries.length;offset+=capacity){check();const batch=queries.slice(offset,offset+capacity),packed=new Float32Array(batch.length*8),fallback=new Set<number>();
        ...
        await staging.mapAsync(GPUMapMode.READ,0,batch.length*16);let result:Float32Array;try{result=new Float32Array(staging.getMappedRange(0,batch.length*16).slice(0));}finally{staging.unmap();}check();stats.transferBytes+=batch.length*48;
        batch.forEach((q,i)=>{const triangle=result[i*4]-1;if(result[i*4+2]!==0||fallback.has(i)||!Number.isFinite(result[i*4+1])){hits.push(cpu(q));stats.refinements++;return;}if(triangle<0){hits.push(null);return;}const data='point'in q?nearestTriangle3(this.source.triangles[triangle],q.point):rayTriangle3(this.source.triangles[triangle],q);if(!data||('point'in q&&data.distance>(q.maxDistance??Infinity))){hits.push(cpu(q));stats.refinements++;}else hits.push(this.source.hit(triangle,data));});
      }
    }finally{input.destroy();output.destroy();staging.destroy();const error=await this.device.popErrorScope();if(error)throw new Error(`WebGPU surface queries: ${error.message}`);}
    stats.wallMs=performance.now()-start;return {hits,stats};
  }
