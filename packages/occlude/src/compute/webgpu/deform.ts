...
export class GpuDeform3 {
  ...
  deform(surface:Surface3,options:DeformOptions3):Promise<{surface:Surface3;stats:{iterations:number;dispatches:number;transferBytes:number;residentBytes:number;wallMs:number}}> {
    const captured=captureDeform3(surface,options),signal=options.signal;
    const job=this.tail.then(()=>this.run(captured,signal));this.tail=job.catch(()=>{});return job;
  }
  private async run(input:ReturnType<typeof captureDeform3>,signal?:AbortSignal){
    ...
    try {
      ...
      await staging.mapAsync(GPUMapMode.READ);let result:Float32Array;try{result=new Float32Array(staging.getMappedRange().slice(0));}finally{staging.unmap();}
      ...
      surface.points.forEach((p,i)=>{if(!input.pinned.has(i))p.position=[0,1,2].map(k=>origin[k]+scale*result[i*4+k]) as unknown as Vec3;});
    ...
    } finally {for(const b of buffers)b.destroy();const error=await this.device.popErrorScope();if(error)throw new Error(`WebGPU deformation: ${error.message}`);}
  }
}
