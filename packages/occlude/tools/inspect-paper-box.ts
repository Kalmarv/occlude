...
const root='../../development/3d/paper-box-oracle/';
...
const cases=variants.map((camera,index)=>{
 ...
 return {index,camera,features:out.features.filter(r=>!r.feature.curve), curves:out.features.filter(r=>r.feature.curve&&r.visible.some(([a,b])=>b-a<1e-10))};
});
writeFileSync(root+'cpu.json',JSON.stringify({paper:classified.frame.paper,objects:scene.objects.map(o=>({id:o.id,surface:o.surface})),cases},null,2));
