...
const reference: {version:string;buildHash:string;cases:{id:string;visibility:'visible'|'hidden';segments:Segment[]}[]} = JSON.parse(readFileSync(resolve(directory,'blender.json'),'utf8'));
assert.equal(reference.version, '5.2.1 LTS');
const hardware: {adapter:{isFallbackAdapter:boolean};cases:{id:string;visibility:string;segments:Segment[]}[]} | undefined = process.argv[3] ? JSON.parse(readFileSync(resolve(process.argv[3]),'utf8')) : undefined;
...
for(const fixture of fixtures){
  ...
  const classified=classifySceneCpu3(featureSnapshot3(objects,[],frame));
  ...
  for(const visibility of fixture.visibility){
    ...
    const result={id:fixture.id,visibility,occludeSegments:segments.length,blenderSegments:expected.segments.length,extraMm,missingMm,passed:extraMm===0&&missingMm===0};cases.push(result);
    const paths=(values:readonly Segment[],color:string)=>values.map(([a,b])=>`<path d="M${a.join(' ')} L${b.join(' ')}" stroke="${color}"/>`).join('');
    writeFileSync(resolve(directory,`${fixture.id}-${visibility}${hardware?'-gpu':''}.svg`),`<svg xmlns="http://www.w3.org/2000/svg" width="200mm" height="100mm" viewBox="0 0 200 100"><rect width="200" height="100" fill="white"/><g fill="none" stroke-width="0.15">${paths(expected.segments,'#a84932')}<g transform="translate(100 0)">${paths(segments,'#18202a')}</g></g></svg>`);
    console.log(JSON.stringify(result));
  }
}
writeFileSync(resolve(directory,hardware?'comparison-gpu.json':'comparison.json'),JSON.stringify({backend:hardware?'gpu':'cpu',referenceVersion:reference.version,buildHash:reference.buildHash,toleranceMm,comparison:'bidirectional collinear segment coverage, independent of chaining',passed:cases.every(c=>c.passed),cases},null,2)+'\n');
