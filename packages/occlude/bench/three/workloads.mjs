...
export default sketchAsync({ seed: 42, pens: { ink: pen({ width: mm(0.2) }) } }, async t => {
  ...
  return view([sheet, marks], { camera: orthographic({ eye: [5, 7, 5], span: 6 }), stroke: 'ink' });
});`},
...
export default sketchAsync({ seed: 42, pens: { ink: pen({ width: mm(0.2) }) } }, async t => {
  ...
  return view([relief, marks], { camera: perspective({ eye: [6, -8, 6], target: [0, 0, 0], fovDegrees: 38 }), stroke: 'ink' });
});`},
...
export default sketchAsync({ seed: 42, pens: { ink: pen({ width: mm(0.2) }) } }, async t => {
  ...
  return view([block, tube, ball, ...seams, marks], { camera: perspective({ eye: [5, 7, 5], target: [0, 0, 0], fovDegrees: 40 }), stroke: 'ink' });
});`},
...
export default sketchAsync({ seed: 42, pens: { ink: pen({ width: mm(0.2) }) } }, async t => {
  ...
  return view([tiles, pattern.place(tiles)], { camera: perspective({ eye: [6, -8, 7], target: [0, 0, 0], fovDegrees: 40 }), stroke: 'ink' });
});`},
...
export default sketchAsync({ seed: 42, pens: { ink: pen({ width: mm(0.2) }) } }, async t => {
  ...
  return view([ball, marks], { camera: orthographic({ eye: [4, -6, 3], span: 4 }), stroke: 'ink' });
});`},
{name:'woven-vessel',src:String.raw`import { sketch, paper, pen, mm, strokes, dash, wobble, label } from 'occlude';
import { cylinder, torus, sphere, polyline, revolve, curve, circle, sweep, view, orthographic } from 'occlude/3d';
...
export default sketch(
  ...
  (t) => {
    ...
    const handle = sweep(rod, curve((u) => { const a = Math.PI * u; return [-(R + 0.02) * Math.cos(a), 0, H + 1.1 * Math.sin(a)]; }, { segments: 72 }), { caps: true });
    const pear = revolve(polyline([[0, 0, -0.5], [0.34, 0, -0.46], [0.4, 0, -0.08], [0.3, 0, 0.28], [0.2, 0, 0.52], [0.18, 0, 0.62], [0.1, 0, 0.68], [0, 0, 0.72]]), { segments: 28 }).translate([0.7, -0.5, 2.5]);
    ...
    return view([plinth, foot, ...weave, band, rim, handle, pear, pip], {
      ...
      hatch: { spacing: mm(1.8), angle: 35, stroke: 'shade', select: (f) => f.ground === true },
      sections: [{ origin: [0, 0, 1.6], normal: [0, 0, 1], stroke: 'measure', key: 'waist' }],
    }, (lines) => {
