...
for(let i=0;i<=Number(samplesArg);i++) {
 ...
 try {
  ...
  if(i) {renderMs.push(generated-start);planMs.push(planned-generated);}
  ...
  const row={name,engine,width:widthArg||'saved pens',paperMm:304.8,seed:42,samples:Number(samplesArg),warmup:1,renderMs:stats(renderMs),planMs:stats(planMs),runs:flat.length,internalLifts:Math.max(0,flat.length-1),inkMm,travelMm,etaMinutes:eta.totalMs/60000,primitives:value.chains.reduce((s,c)=>s+c.prims.length,0),planBytes:buffer.byteLength,maxGap,diagnostics:result.stats.contour};
