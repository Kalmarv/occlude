...
export default sketch({ aspect: 'square', margin: 8, seed: 'url' }, ({ line, circle, modify, wobble, dash, decimate, hatch, times, rnd }) => {
  const nasty = (x: number, y: number) => x < 20 ? -1 : x < 40 ? 2 : x < 60 ? NaN : x < 80 ? Infinity : y / 100;
  const scene = [
    times(40, (k, t) => line(0, t * 100, 100, t * 100)),
    times(8, () => circle(rnd(100), rnd(100), rnd(5, 15), { fill: hatch(rnd(180), mm(1)) })),
  ];
  return [
