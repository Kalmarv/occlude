...
const instanceCheck=process.env.OCCLUDE_API_EXAMPLE==='instances',queryCheck=process.env.OCCLUDE_API_EXAMPLE==='queries',curveCheck=process.env.OCCLUDE_API_EXAMPLE==='curves';
...
try{
 const page=await browser.newPage({viewport:{width:1500,height:1100}});let models=0;const shared=[];const queries=[];const curveOracles=[];const errors=[];
 page.on('console',m=>{if(m.text().startsWith('curve-oracle:'))curveOracles.push(JSON.parse(m.text().slice('curve-oracle:'.length)));if(m.text()==='mesh-model')models++;if(m.text().startsWith('instance-shared:'))shared.push(Number(m.text().split(':')[1]));if(m.text().startsWith('query-proof:'))queries.push(JSON.parse(m.text().slice('query-proof:'.length)));});page.on('pageerror',e=>errors.push(String(e)));
 ...
 const probe=curveCheck?"circle().attribute('height',1).points.at(0).height":queryCheck?"query(plane()).batch().nearest(plane().attribute('height',1).points)[0].source.height":instanceCheck?"instanceOnPoints(cone(),pointCloud([[0,0,0]]).attribute('height',1).points).instances.at(0).height":"plane().attribute('height',1).points.at(0).height";
 await page.evaluate(({probe,queryCheck,curveCheck})=>window.__occlude.editor.setValue(window.__occlude.editor.getValue()+'\nconst __typeProbe: string = '+probe+';'+(queryCheck?"\nconst __faceProbe: string = query(plane().faceAttribute('roof',true)).nearest([0,0,1]).face.roof;":'')+(curveCheck?'\ncircle().faces();':'')),{probe,queryCheck,curveCheck});
 ...
 if(queryCheck)assert.equal(negative.filter(d=>d.code===2322).length,2);
 if(curveCheck){
...
return strokes(lines.visible);
 ...
 const report={passed:true,base,diagnostics,negative,modelsBeforeReopen:1,modelsAfterReopen:2,reopened,stats:before.stats,zeroLengthPaths,sharedPrototypeCounts:shared,queries,curveOracles,errors};await writeFile(output+'/report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
