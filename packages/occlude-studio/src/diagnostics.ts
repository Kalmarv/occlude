 ...
 * `to` (bottom), 40 mm long, 6 mm apart, each pen index pinned to that
 * pen-down height through the driver's override hook. Read off the paper:
 * the first stroke that is full weight is the machine's pen-down Z;
 * anything deeper only presses. */
export function zLadder(base: PenDef | undefined, o: { from: number; to: number; steps?: number }): Diagnostic {
  ...
  const pens: PenDef[] = [];
  const servo: ServoOverride[] = [];
  const chains: Chain[] = [];
  for (let i = 0; i < n; i++) {
    const z = Math.round((o.from + ((o.to - o.from) * i) / (n - 1)) * 100) / 100;
    pens.push(pen(`z-${z.toFixed(2)}`, base?.feed ?? 3000, base));
    servo.push({ down: z });
    const y = 6 + i * 6;
