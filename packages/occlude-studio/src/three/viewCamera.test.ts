...
export default sketch({ seed: 1 }, () => [
  ...
  view(plane(2), { camera: orthographic({ eye: [5, 7, 6], span: 6 }), pen: 'ink' }),
]);
`;
  ...
  expect(out).toContain(`view(plane(2), { camera: perspective({ eye: [1, 2, 3], target: [0, 0, 0], up: [0, 1, 0], fovDegrees: 40 }), pen: 'ink' })`);
...
export default sketch({}, () => show(box(1), { pen: 'ink' }));
`;
